...
$('#erase').onclick=()=>{if(state.selected==null){showToast('Bitte zuerst ein Feld auswählen.');return}const i=state.selected;if(!state.hints.has(i)&&!editableSelected()){showToast('Dieses Feld kann nicht gelöscht werden.');return}state.hints.delete(i);state.board[i]=0;if(state.mode==='manual'){if(!state.manualLocked){state.solution=null;state.solutionCount=0}else refreshManualSolutionFromCurrentBoard()}saveDaily();render()};
